...
# 2. MVVM 패턴
1. Model이 비지니스 데이터를 관리하다가 값이 변경되면 ViewModel(이하 VM)에게 변경 사항을 알린다.
2. VM은 Model의 변경 내용을 화면인 View에게 알려서 반영하므로 사용자가 내용을 확인할 수 있게 한다.
3. View를 보던 사용자가 어떤 입력 값을 변경하면 VM에게 데이터 바인딩에 대한 요청을 보낸다.
4. VM은 실제 Model의 값을 변경하면서 데이터 일관성을 유지한다.
