...
# 2. MVVM 패턴
![mvvm](./mvvm.PNG) 
1. Model이 비지니스 데이터를 관리하다가 값이 변경되면 ViewModel에게 변경 사항을 알린다.
2. ViewModel은 Model의 변경 내용을 화면인 View에게 알려서 반영하므로 사용자가 내용을 확인할 수 있게 한다.
3. View를 보던 사용자가 어떤 입력 값을 변경하면 ViewModel에게 데이터 바인딩에 대한 요청을 보낸다.
4. ViewModel은 실제 Model의 값을 변경하면서 데이터 일관성을 유지한다.
...
![Incremental DOM](./Incremental DOM.PNG)     
실제 DOM을 사용하여 코드의 변경 사항을 찾는 방식
- 모든 컴포넌트는 일련의 명령으로 컴파일 된다. 이 명령들은 데이터가 변경될 때 DOM 트리를 만들고 업데이트 한다.
...
- template 에서 사용하고자하는 변수, 함수등을 정의한뒤 return 해주면 template 에서 해당 변수와 함수를 접근할수있다.
